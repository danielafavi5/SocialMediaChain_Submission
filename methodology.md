...
3. It fetches the file from the platform's CDN via an HTTP GET call and stores the raw bytes in memory. 
   * *Note on Platform-Specific Behavior*: In this API-driven sandbox, **Telegram always forces server-side recompression**, stamping the image with its custom Q-table and clearing EXIF metadata. In contrast, **Discord serves the uploaded attachment completely losslessly**, and **Slack serves it losslessly unless the image exceeds its client-side 4MB pre-compression safeguard size**.
4. Those bytes become the *input* for the next platform in the sequence. Because Discord and Slack act as lossless hosts while Telegram forces recompression, the final Step 3 image in every sequence retains the Telegram Q-table (or Slack's pre-compressed table if triggered). This creates a zero-variance Q-table feature set at Step 3, meaning that the model cannot rely on Step 3 Q-tables to identify the surface platform, and must instead rely on sequential learning (Classifier Chains) and robust spatial/statistical invariants (Markov transitions, Benford's Law).
...
**Benford's Law Distribution Analysis (9 dimensions).** We analyze the leading-digit probability distribution of all non-zero high-frequency AC DCT coefficients. Under natural image statistics, the frequency of leading digits follows Benford's Law (digit 1 appears ~30% of the time, digit 9 approximately 5%). Successive platform re-quantization perturbs this distribution in a platform-dependent direction. These 9 probability bins provide a non-divisibility feature that provides discriminative signal even after low-coefficient quantization, as the shape of the distribution encodes prior-round structure independent of absolute coefficient magnitudes.
...
- **BKS Fusion:** A deterministic Q-table divisibility layer in `core/bks_fusion.py` that overrides predictions when an integer-ratio relationship is detected. To address a mathematical bias where low-quantization tables (such as Slack/Discord with mostly 1s and 2s) naturally generate low L1 divisibility errors, the engine implements **Non-Trivial Divisor Masking**. The L1 error is calculated strictly on coefficients where the candidate table contains values greater than 1, requiring a minimum of 8 non-trivial bins to validate the traceback.
...
The current architecture replaces the modular C1/C2/BKS stack with a **Classifier Chain** that models the platform sequence as a dependent timeline.

**Training input & Evaluation Leakage**: For each chain in the training set, the 272-dimensional feature vectors of Step 1, Step 2, and Step 3 images are concatenated into a single **816-dimensional chain signature**, providing the complete sequence history as input. This concatenated format assumes visibility of all intermediate images. While it is useful for training-pipeline validation, a blind sequence model operating strictly on the final Step 3 image's 272-dimensional feature vector (without intermediate features) achieves an exact sequence match accuracy of **11.1%** due to lossy signature overwriting.

**Training target & chain mechanism**: The model predicts `[platform_step1, platform_step2, platform_step3]` simultaneously. Using Scikit-Learn's `ClassifierChain`, we train three Random Forest classifiers (`n_estimators=500`, `max_features=0.2`). The Step 2 classifier receives the 816-dimensional signature alongside the predicted label of Step 1. The Step 3 classifier receives the 816-dimensional signature alongside the predicted labels of Step 1 and Step 2. This structure models inter-step sequence dependencies.

**CLI Predictor Fallback**: Because the multi-output sequence model (`seq_model.joblib`) requires the 816-dimensional concatenated vector of three distinct images, the user-facing inference script (`analyze_image.py`) cannot execute it on a single user-supplied image. Instead, `analyze_image.py` is decoupled from the sequence chain and falls back to a 2-step single-image pipeline: predicting the surface platform via `surface_model.joblib` and back-tracking a single prior step via BKS Q-table divisibility.

**Result**: This sequential classification structure increased the exact 3-step chain reconstruction accuracy in the concatenated validation sandbox from **4.5%** (modular BKS fused baseline) to **87.5%**, with Step 1 accuracy reaching **97.2%**, demonstrating the statistical importance of modeling multi-step dependencies when intermediate states are accessible.
