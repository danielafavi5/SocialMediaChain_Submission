...
against the ENTIRE dataset (all 333 complete chains, training + test combined).

Purpose: show the absolute capacity ceiling of the model — i.e. how well the
ClassifierChain has learned to map compression signatures to platform sequences
across both seen (training) and unseen (test) data.

Compare with reproduce_results_offline.py which reports the official
generalization metric on the 67 strictly held-out test chains only.

Expected output:
  - Full dataset (333 chains): ~91.9% exact match  (capacity ceiling)
  - Held-out test only (67 chains): ~74.6% exact match  (true generalization)
"""
...
def main():
    ...
    # Build ground truth dictionary for all complete 3-step chains
    gt_chains = {
        entry["chain_id"]: entry.get("sequence", [])
        for entry in manifest
        if entry.get("chain_id") and len(entry.get("sequence", [])) == 3
    }

    # Locate all images in the samples directory (train + test, no filtering)
    ...
    print(f"  Total chains found in samples/: {len(chains)}")
    print("\nStarting Parallel Full Dataset Evaluation (Train + Test, capacity ceiling)...")
    ...
    print("\n" + "="*60)
    print(f"  [DIAGNOSTIC] Total Chains Evaluated: {total} (Train + Test)")
    ...
    print("="*60)
    print("\n  NOTE: This includes training set chains.")
    print("  For the official generalization metric, run:")
    print("  python scripts/reproduce_results_offline.py\n")
